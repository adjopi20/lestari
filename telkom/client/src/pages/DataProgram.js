...
const DataProgram = () => {
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        const response = await axios.get("https://lestariku.com/api/programs");
    ...
    const fetchCounts = async () => {
      try {
        const response = await axios.get(
          "https://lestariku.com/api/count-programs"
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://lestariku.com/api/programs/${id}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (editingItem) {
        ...
        await axios.put(`https://lestariku.com/api/programs/${formData.id}`, {
          program: formData.program,
        });
        setPrograms(
          programs.map((item) =>
            item.id === formData.id
              ? { ...item, program: formData.program }
              : item
          )
        );
      } else {
        ...
        const response = await axios.post(
          "https://lestariku.com/api/programs",
          { program: formData.program }
        );
      ...
      <div className="data-program">
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="data-program-form">
            ...
            <button type="submit">
              {editingItem ? "Update Data" : "Tambah Data"}
            </button>
