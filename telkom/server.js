...
app.post("/api/submit", upload.single("file"), (req, res) => {
  ...
  const query =
    "INSERT INTO submissions (namaLop, unitBisnis, lokasi, program, ketKlarifikasi, namaWaspang, v4Checklist, file, user_id, status, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  db.query(
    query,
    [
      namaLop,
...
app.get("/api/admin/overview", (req, res) => {
  ...
  const rejectedSubmissionsQuery =
    "SELECT COUNT(*) AS count FROM submissions WHERE status='Rejected'";
  const approvedSubmissionsQuery =
